...
export class Input {
  constructor() {
    ...
    window.addEventListener('keyup', (e) => {
      ...
      if (!a) return;
      this.held[a] = false;
      if (!this.down(a)) this.releasedNow[a] = true;
    });
    window.addEventListener('blur', () => { this.held = {}; });
  }

  // Standard-mapping gamepad: dpad/left stick, A=pass, X/B=throw, Start=start.
  pollGamepad() {
    ...
    for (const a of Object.keys(now)) {
      ...
      if (!now[a] && this.gpHeld[a] && !this.held[a] && !this.touchHeld[a]) this.releasedNow[a] = true;
    }
    this.gpHeld = now;
  }
  ...
  setTouch(a, on) {
    ...
    if (!on && this.touchHeld[a] && !this.held[a] && !this.gpHeld[a]) this.releasedNow[a] = true;
    this.touchHeld[a] = on;
  }
  ...
  endFrame() { this.pressedNow = {}; this.releasedNow = {}; }
}
...
export function initTouch(input) {
  ...
  mkBtn('THROW', 'throw', 'b-throw');
  mkBtn('PASS', 'pass', 'b-pass');
